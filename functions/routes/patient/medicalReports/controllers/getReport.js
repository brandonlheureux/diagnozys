require("dotenv").config("dev");
const { MongoClient } = require("mongodb");

/**
 * @controller get report - will retrieve report from db
 * ---
 * @param reportId - the report id to query and return
 * ---
 * @response the medical report
 */
const getReport = async (req, res, next) => {
  // get user id
  const {
    user: { sub: userId },
    params: { reportId },
  } = req;

  // sanity check for userId and params
  if (!userId) {
    return res.status(401).json({ error: "Unauthorized - no user id" });
  } else if (!reportId || reportId === "undefined") {
    return res.status(400).json({ error: "Missing report id" });
  }

  const client = new MongoClient(process.env.MONGODB_URI, {
    useNewUrlParser: true,
    useUnifiedTopology: true,
  });

  let errorCode = 500;

  try {
    await client.connect();

    // set up collection
    const medicalReports = client.db("medical").collection("reports");

    // get user profile
    const medicalReport = await medicalReports.findOne({ _id: reportId });

    if (!medicalReport) {
      errorCode = 404;
      throw new Error("Could not find medical report");
    } else if (medicalReport.userId !== userId) {
      errorCode = 403;
      throw new Error("Unauthorized medical report access");
    } else {
      // send medical report to client
      res.status(200).json({ medicalReport });
    }
  } catch (e) {
    res.status(errorCode).json({ error: await e.toString() });
  } finally {
    client.close();
  }
};

module.exports = { getReport };
